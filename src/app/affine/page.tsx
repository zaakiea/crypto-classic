...
import { affine, toAlpha, CipherStep } from "@/lib/ciphers";
...
export default function AffineScreen() {
    ...
    function runCipher(m: "encrypt" | "decrypt" = mode) {
        try {
            const { result, steps: cipherSteps } = affine(inputText, aVal, bVal, m);
        ...
        <div className="w-full h-full aero-window flex flex-col shadow-[2px_10px_30px_rgba(0,0,0,0.5)] overflow-hidden">
            ...
            <div className="aero-titlebar flex items-center justify-between px-3 py-1.5 select-none relative z-10">
                <div className="flex items-center gap-2">
                    <span className="material-symbols-outlined text-[16px] text-[#003366] drop-shadow-[0_1px_1px_rgba(255,255,255,0.8)]">lock</span>
                    <span className="text-sm font-semibold text-[#000] drop-shadow-[0_1px_2px_rgba(255,255,255,0.8)] tracking-wide">
                        Affine Cipher - Kripto Klasik
                    </span>
                </div>
            ...
            <div className="aero-content flex-1 p-5 bg-[#f0f0f0] flex flex-col gap-5 overflow-y-auto">
                ...
                <div className="flex bg-white aero-inset p-4 gap-4 items-start">
                    ...
                    <div>
                        <h2 className="text-[#003399] font-semibold text-base mb-1">Affine Cipher Encryption Module</h2>
                        <p className="text-xs text-slate-700">
                            Metode enkripsi substitusi monoalfabetik menggunakan fungsi matematika linear <code className="bg-[#e0e0e0] px-1 rounded border border-[#c0c0c0]">(ax + b) mod 26</code>.
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
                    ...
                    <div className="lg:col-span-5 flex flex-col gap-4 pb-4 lg:pb-0">
                        <fieldset className="border border-[#b5b5b5] p-4 bg-white/40 shadow-[inset_0_1px_0_rgba(255,255,255,1)] h-full">
                            ...
                            <div className="flex flex-col gap-5 mt-2">
                                <label className="flex flex-col gap-1.5">
                                    <span className="text-xs text-slate-800">Nilai a (Koprim ke 26):</span>
                                    <input
                                        className="w-full aero-inset py-1.5 px-2 text-[13px] text-slate-900 focus:outline-none focus:bg-[#fafffb]"
                                        type="number"
                                        value={aVal}
                                        onChange={(e) => setAVal(parseInt(e.target.value) || 1)}
                                    />
                                    <p className="text-[10px] text-slate-500 italic mt-0.5">
                                        * Bilangan ganti selain 13 (cth: 1, 3, 5, 7, 9, 11...).
                                    </p>
                                </label>

                                <label className="flex flex-col gap-1.5">
                                    <span className="text-xs text-slate-800">Nilai b (Pergeseran):</span>
                                    <input
                                        className="w-full aero-inset py-1.5 px-2 text-[13px] text-slate-900 focus:outline-none focus:bg-[#fafffb]"
                                        type="number"
                                        value={bVal}
